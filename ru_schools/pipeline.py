    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                continue
            except (ServiceMaintenance, CaptchaRequired):
                # Капча и «сервис недоступен» — это состояние сервиса, а не
                # свойство организации: отметку ставить нельзя, иначе после
                # пяти таких ответов школа выбывает из сбора навсегда.
                # Разовый отказ не повод бросать партию — только череда.
                maintenance += 1
                if maintenance >= MAINTENANCE_LIMIT:
                    log.warning(
                        "ЕГРЮЛ отказывает подряд %s раз (техработы или капча) "
                        "— этап приостановлен, продолжится позже",
                        maintenance,
                    )
